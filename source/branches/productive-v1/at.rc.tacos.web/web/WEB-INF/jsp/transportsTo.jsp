	...
	<c:when test="${fn:length(transportsToMap) gt 0}">
		<table cellpadding="3" cellspacing="0" class="list">
			<c:forEach var="transportsToMapEntry" items="${transportsToMap}">
				...
				${fieldHeadersRow}
				<c:forEach var="transportsToContainer"
					items="${transportsToMapEntry.value}" varStatus="loop">
					<tr class="${loop.count % 2 == 0 ? 'even' : 'odd'}">
						<td nowrap="nowrap">${transportsToContainer.fromStreet}
						&nbsp; ${transportsToContainer.fromCity}</td>
						<td nowrap="nowrap">${transportsToContainer.patient.lastname}
						&nbsp; ${transportsToContainer.patient.firstname }</td>
						<td nowrap="nowrap">${transportsToContainer.toStreet} &nbsp;
						${transportsToContainer.toCity }</td>
						<td nowrap="nowrap"><fmt:formatDate type="both"
							timeStyle="short"
							value="${transportsToContainer.plannedStartOfTransport}" />
						<td nowrap="nowrap"><fmt:formatDate type="time"
							timeStyle="short"
							value="${transportsToContainer.plannedTimeAtPatient}" />
						<td nowrap="nowrap"><fmt:formatDate type="time"
							timeStyle="short"
							value="${transportsToContainer.appointmentTimeAtDestination}" /></td>
						<td nowrap="nowrap">${transportsToContainer.notes}</td>
						<td nowrap="nowrap"><c:choose>
							<c:when test="${transportsToContainer.assistantPerson eq true}">Ja</c:when>
