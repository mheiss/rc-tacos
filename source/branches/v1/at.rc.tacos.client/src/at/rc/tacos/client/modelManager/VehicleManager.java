...
public class VehicleManager extends PropertyManager implements PropertyChangeListener, ITransportStatus, IProgramStatus {
	...
	/**
	 * Adds a new vehicle to the vehicle manager. This class is thread save,
	 * that means adding new vehicles, can be done from any thread.
	 * 
	 * @param vehicle
	 *            the vehicle to add
	 */
	...
	public void propertyChange(PropertyChangeEvent evt) {
			...
			if (transport.getNotes() != null) {
				if (transport.getProgramStatus() == PROGRAM_STATUS_OUTSTANDING && transport.getNotes().contains("Fahrzeugabzug")) {
					...
					for (VehicleDetail detachedVehicle : vehicleList) {
						...
						List<Transport> vehicleDetachedTransportList = transportManager.getUnderwayTransportsByVehicle(detachedVehicle
								.getVehicleName());
		...
		if ("ROSTERENTRY_UPDATE".equalsIgnoreCase(evt.getPropertyName())) {
			...
			// show a message box
			MessageDialog.openInformation(Display.getDefault().getActiveShell(), "Information", "Der Mitarbeiter " + member.getFirstName() + " "
					+ member.getLastName() + " hat sich abgemeldet.\n" + "Er wurde vom Fahrzeug " + detail.getVehicleName() + " abgezogen");
		}
	}
