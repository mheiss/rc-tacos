...
public class TransportManager extends PropertyManager implements ITransportStatus, IProgramStatus {
		...
		Display.getDefault().syncExec(new Runnable() {
			...
			public void run() {
				...
				if (!objectList.contains(transport)) {
					// if direct car assigning - copy the transport into the
					// clipboard
					if (transport.getTransportNumber() != 0) {
						if (transport.getVehicleDetail() != null) {
							// NEF should not override the transport number of
							// the amblance
							if (!transport.getVehicleDetail().getVehicleName().equalsIgnoreCase("NEF")) {
								CopyTransportDetailsIntoClipboardUpdateAction clipboardAction = new CopyTransportDetailsIntoClipboardUpdateAction(
										transport);
								clipboardAction.run();
							}
						}
	...
	public void update(final Transport transport) {
		Display.getDefault().syncExec(new Runnable() {
			...
			public void run() {
				...
				if (!objectList.contains(transport))
					return;
				// copy the transport details into the clipboard if the
				// 'assignCarAction' was executed
				...
				// check if the old transport has no vehicle, but the new has
				// one
	...
	/**
	 * Informs the views that the selected date in the transport view filter has
	 * changed.
	 * 
	 * @param newDate
