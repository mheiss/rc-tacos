...
/**
 * Starts a background thread that checks every minutes the status of the
 * transports. When a prebooked transport is within the next 2 hours then the
 * thread updates the transport and moves it to the outstanding transports by
 * changing the status and sending a update request
 */
	...
	protected IStatus run(IProgressMonitor monitor) {
		try {
			...
			for (DialysisPatient patient : ModelFactory.getInstance().getDialyseManager().getDialysisList()) {
				...
				// first check: do we have already generated a transport for
				// today?
			...
			for (DialysisPatient patient : ModelFactory.getInstance().getDialyseManager().getDialysisList()) {
				Calendar currentDialysis = Calendar.getInstance();
				// first check: do we have already generated a back transport
				// for today?
			...
			// start again in a minute plus a random time so that different
			// clients use different times
