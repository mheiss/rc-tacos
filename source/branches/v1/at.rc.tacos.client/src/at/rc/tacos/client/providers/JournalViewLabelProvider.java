...
public class JournalViewLabelProvider implements ITableLabelProvider, ITableColorProvider, ITransportStatus, ITableFontProvider {
	...
	public Image getColumnImage(Object element, int columnIndex) {
		...
		switch (columnIndex) {
			...
			case COLUMN_TRANSPORT_FROM:
				...
				cal4.set(1970, 01, 01);
				cal4.set(Calendar.HOUR_OF_DAY, cal4.get(Calendar.HOUR_OF_DAY) - 4);// show
																					// all
																					// possible
																					// back
																					// transports
			...
			case COlUMN_OS:
				...
				if (transport.getVehicleDetail() != null) {
					if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Kapfenberg"))
						...
						return "BR";
					else if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Thörl")
							|| transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Th?rl"))
						return "TH";
			...
			case COLUMN_PARAMEDIC_I:
				if (transport.getVehicleDetail() != null) {
					if (transport.getVehicleDetail().getFirstParamedic() != null) {
						return transport.getVehicleDetail().getFirstParamedic().getLastName() + " "
								+ transport.getVehicleDetail().getFirstParamedic().getFirstName();
					}
					else
						return "";
				}
			case COLUMN_PARAMEDIC_II:
				if (transport.getVehicleDetail() != null) {
					if (transport.getVehicleDetail().getSecondParamedic() != null) {
						return transport.getVehicleDetail().getSecondParamedic().getLastName() + " "
								+ transport.getVehicleDetail().getSecondParamedic().getFirstName();
					}
					else
						return "";
				}
