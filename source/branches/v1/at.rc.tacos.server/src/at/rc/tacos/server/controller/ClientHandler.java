...
public class ClientHandler implements INetListener {
	...
	public void dataReceived(NetEvent ne) {
		...
		try {
			...
			SystemMessage system = new SystemMessage("Ausnahmefehler: Die zuletzt gesendete Anforderung kann nicht bearbeitet werden",
					SystemMessage.TYPE_ERROR);
			// setup the message
		...
		try {
			...
			if (IModelActions.LOGOUT.equalsIgnoreCase(queryString)) {
				...
				// remove the client form the list of authenticated clients if
				// successfully
		...
		// catch all sql errors that occured during the operations with the
		// listener classes
