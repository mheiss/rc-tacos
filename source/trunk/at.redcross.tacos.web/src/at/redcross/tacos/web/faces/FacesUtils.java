...
public class FacesUtils {
    ...
    public static void redirectError(Throwable t) {
        ...
        redirect("error", "?cameFrom=" + requestedUri, "&errorCode=" + resolver.resolve());
    }
     ...
     */
    public static void redirect(String pageId, String... pageParams) {
        FacesContext facesContext = FacesContext.getCurrentInstance();
        PrettyContext prettyContext = PrettyContext.getCurrentInstance();
        ExternalContext externalContext = facesContext.getExternalContext();
        try {
            PrettyConfig prettyConfig = prettyContext.getConfig();
            PrettyUrlMapping mapping = prettyConfig.getMappingById(pageId);
            ...
            for (String pageParam : pageParams) {
                target.append(pageParam.trim());
            }
            facesContext.responseComplete();
            externalContext.redirect(target.toString());
        } catch (Exception e) {
            log.fatal("Failed to redirect to '" + pageId + "'", e);
        }
    }
