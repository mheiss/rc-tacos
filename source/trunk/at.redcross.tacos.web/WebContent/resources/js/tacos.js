...
function smartGrid() {
	...
	var colNum = Math.floor(colWrap / 300); // Find how many columns of 300px
											// can fit per row / then round it
											// down to a whole number
	var colFixed = Math.floor(colWrap / colNum); // Get the width of the row
													// and divide it by the
													// number of columns it can
													// fit / then round it down
													// to a whole number. This
													// value will be the exact
													// width of the re-adjusted
													// column
...
function limitChars(editor, infoDiv, limit) {
	...
	if (contentLength > limit) {
		...
		jQuery('#' + infoDiv)
				.html(
						'Sie können noch ' + (limit - contentLength) + ' Zeichen verwenden.');
		return true;
	}
}
