...
public class CompetenceHelper {
	...
	public static List<Competence> list(EntityManager manager) {
		return manager.createQuery("from Competence", Competence.class)
				.getResultList();
	}

	public static Competence getByName(EntityManager manager, String name) {
		String hqlQuery = "from Competence c where c.name = :name";
		TypedQuery<Competence> query = manager.createQuery(hqlQuery,
				Competence.class);
