...
import at.rc.tacos.platform.net.request.Request;
...
public class XmlProtocolDecoder extends TextLineDecoder {
	...
	protected void writeText(IoSession session, String text, ProtocolDecoderOutput out) {
		// ttry to decode the message
		try {
			Object message = xStream.fromXML(text);
			// assert we are receiving only request messages
			if (!(message instanceof Request)) {
				logAndThrowException("Can only send messages from type 'Request' is " + message == null ? "null" : message.getClass().getName(), null);
			}
