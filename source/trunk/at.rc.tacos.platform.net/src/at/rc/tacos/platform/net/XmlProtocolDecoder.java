...
public class XmlProtocolDecoder extends TextLineDecoder {
    ...
    /**
     * Coverts the received line of text to a higher level object and propagates it to the {@code
    ...
    protected void writeText(IoSession session, String text, ProtocolDecoderOutput out) {
        ...
        try {
            ...
            RequestHeader header = (RequestHeader) messageHeader;

            // save the command in the current session
            session.setAttribute(Constants.UNIQUE_ID, header.getId());
            session.setAttribute(Constants.COMMAND, header.getCommand());
            session.setAttribute(Constants.PARAMS, header.getParams());
        } catch (XStreamException xse) {
            logAndThrowException("Failed to decode the message header: " + xse.getMessage(), xse
                    .getCause());
        }
        ...
        try {
            ...
            logAndThrowException("Failed to decode the message body: " + xse.getMessage(), xse
                    .getCause());
        }
    }
