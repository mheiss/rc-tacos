...
/**
 * The address handler is responsible for request that are made agains address objects.
 * 
 * @author Michael
 */
public class AddressHandler implements Handler<Address> {
    ...
    @Override
    public void add(ServerIoSession session, Message<Address> model) throws ServiceException,
            SQLException {
        List<Address> addressList = model.getObjects();
        ...
        session.writeBrodcast(model, addressList.toArray());
    }

    @Override
    public void get(ServerIoSession session, Message<Address> model) throws ServiceException,
            SQLException {
        ...
        // the query filter
        final String addressFilter = "Street: " + streetFilter + " | City: " + cityFilter
                + " | Zip: " + zipFilter;
        log.debug(addressFilter);

        List<Address> addressList = addressService.getAddressList(streetFilter, streetNumberFilter,
                cityFilter, zipFilter);
        if (addressList == null)
    ...
    @Override
    public void remove(ServerIoSession session, Message<Address> model) throws ServiceException,
            SQLException {
        List<Address> addressList = model.getObjects();
        // loop and try to add each address object
        for (Address adr : addressList) {
    ...
    @Override
    public void update(ServerIoSession session, Message<Address> model) throws ServiceException,
            SQLException {
        List<Address> addressList = model.getObjects();
        // loop and try to add each address object
        for (Address adr : addressList) {
    ...
    @Override
    public void execute(ServerIoSession session, Message<Address> message) throws SQLException,
            ServiceException {
