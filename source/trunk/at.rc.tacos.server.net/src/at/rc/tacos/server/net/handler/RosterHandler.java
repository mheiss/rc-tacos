...
public class RosterHandler implements Handler<RosterEntry> {
    ...
    @Override
    public void add(ServerIoSession session, Message<RosterEntry> message) throws ServiceException,
            SQLException {
    ...
    @Override
    public void get(ServerIoSession session, Message<RosterEntry> message) throws ServiceException,
            SQLException {
        // get the params of the request
        Map<String, String> params = message.getParams();

        if (params.containsKey(IFilterTypes.DATE_FILTER)
                && params.containsKey(IFilterTypes.ROSTER_LOCATION_FILTER)) {
            ...
            int filterLocationId = Integer
                    .parseInt(params.get(IFilterTypes.ROSTER_LOCATION_FILTER));
            List<RosterEntry> rosterList = rosterService.listRosterEntriesByDateAndLocation(
                    dateStart, dateEnd, filterLocationId);
            if (rosterList == null) {
        ...
        if (params.containsKey(IFilterTypes.ROSTER_MONTH_FILTER)
                && params.containsKey(IFilterTypes.ROSTER_YEAR_FILTER)) {
            ...
            if (params.containsKey(IFilterTypes.ROSTER_FUNCTION_STAFF_MEMBER_COMPETENCE_FILTER)) {
                functionStaffMemberCompetenceFilter = params
                        .get(IFilterTypes.ROSTER_FUNCTION_STAFF_MEMBER_COMPETENCE_FILTER);
            }
            if (params.containsKey(IFilterTypes.ROSTER_LOCATION_STAFF_MEMBER_FILTER)) {
                locationStaffMemberFilter = Integer.parseInt(params
                        .get(IFilterTypes.ROSTER_LOCATION_STAFF_MEMBER_FILTER));
            }
            int staffMemberFilter = -1;
            if (params.containsKey(IFilterTypes.ROSTER_STAFF_MEMBER_FILTER)) {
                staffMemberFilter = Integer.parseInt(params
                        .get(IFilterTypes.ROSTER_STAFF_MEMBER_FILTER));
            }
            ...
            if (params.containsKey(IFilterTypes.ROSTER_SERVICE_TYPE_FILTER)) {
                serviceTypeFilter = Integer.parseInt(params
                        .get(IFilterTypes.ROSTER_SERVICE_TYPE_FILTER));
            }
            List<RosterEntry> rosterList = rosterService.listRosterEntriesForRosterMonth(
                    locationFilter, monthFilter, yearFilter, locationStaffMemberFilter,
                    functionStaffMemberCompetenceFilter, staffMemberFilter, statisticFilter,
                    serviceTypeFilter);
            if (rosterList == null) {
    ...
    @Override
    public void remove(ServerIoSession session, Message<RosterEntry> message)
    ...
    @Override
    public void update(ServerIoSession session, Message<RosterEntry> message)
            throws ServiceException, SQLException {
        List<RosterEntry> rosterList = message.getObjects();
    ...
    @Override
    public void execute(ServerIoSession session, Message<RosterEntry> message)
            throws ServiceException, SQLException {
