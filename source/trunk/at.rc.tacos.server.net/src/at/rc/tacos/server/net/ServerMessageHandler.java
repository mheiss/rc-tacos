    ...
    public void messageReceived(MessageIoSession session, Message<Object> message) throws Exception {
        ...
        if (connection == null)
            throw new ServiceException(
                    "Failed to get a valid database connection, the data source returned null");
        ...
        List<Object> resolvedSources = dataSourceResolver.resolveAnnotations(resolvedServices
                .toArray());
        ...
        if (log.isDebugEnabled()) {
            log.debug("Resolving the request agains " + handler.getClass().getSimpleName()
                    + " took " + (end - start) + " ms");
        }
