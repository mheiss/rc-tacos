...
public class AddressEditor extends EditorPart implements PropertyChangeListener
{
	...
	private void createManageSection(Composite parent)
	{
		...
		removeHyperlink.addHyperlinkListener(new HyperlinkAdapter()
		{
			...
			public void linkActivated(HyperlinkEvent e) 
			{
				...
				NetActivator.getDefault().sendRemoveMessage(Address.ID, address);
			}
		});
	...
	public void doSave(IProgressMonitor arg0) 
	{
		...
		if(isNew)
		{
			NetActivator.getDefault().sendAddMessage(Address.ID, address);
		}
		else
		{
			NetActivator.getDefault().sendUpdateMessage(Address.ID, address);
		}
	}
