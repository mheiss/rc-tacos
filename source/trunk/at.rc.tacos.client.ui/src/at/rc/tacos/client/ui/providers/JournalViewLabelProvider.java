	...
	public Image getColumnImage(Object element, int columnIndex) {
		...
		switch (columnIndex) {
			...
			case COLUMN_TRANSPORT_FROM:
				...
				cal5.set(1970, 01, 01);// necessary because of the different
				// year/month/day format in the database
				...
				// all possible back transports within
				// 4 hours get an arrow
				if (cal5.getTimeInMillis() > cal4.getTimeInMillis())
					return imageRegistry.get("transport.backtransport");
				// no image
				return null;
				// default image -> none
			default:
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		switch (columnIndex) {
			...
			case COLUMN_LOCK:
				return null;
			case COlUMN_OS:
				// return the current station of the vehicle if there is one
				if (transport.getVehicleDetail() == null) {
					return null;
				}
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Kapfenberg"))
					return "KA";
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Bruck an der Mur"))
					return "BM";
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("St. Marein"))
					return "MA";
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Breitenau"))
					return "BR";
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Thörl"))
					return "TH";
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Turnau"))
					return "TU";
				if (transport.getVehicleDetail().getCurrentStation().getLocationName().equalsIgnoreCase("Bezirk: Bruck - Kapfenberg"))
					return "BE";
			case COLUMN_TRANSPORT_NUMBER:
				if (transport.getTransportNumber() == Transport.TRANSPORT_CANCLED)
					return "STORNO";
				if (transport.getTransportNumber() == Transport.TRANSPORT_FORWARD)
					return "WTGL";
