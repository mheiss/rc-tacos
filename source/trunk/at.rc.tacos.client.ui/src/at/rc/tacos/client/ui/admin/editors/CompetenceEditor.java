...
public class CompetenceEditor extends EditorPart implements DataChangeListener<Competence> {
	...
	private void createManageSection(Composite parent) {
		...
		removeHyperlink.setImage(UiWrapper.getDefault().getImageRegistry().get("admin.competenceRemove"));
		removeHyperlink.addHyperlinkListener(new HyperlinkAdapter() {

			@Override
			public void linkActivated(HyperlinkEvent e) {
				boolean result = MessageDialog.openConfirm(getSite().getShell(), "Löschen der Kompetenz bestätigen", "Möchten sie die Kompetenz "
						+ competence.getCompetenceName() + " wirklich löschen?");
				if (!result)
					return;
				...
				RemoveMessage<Competence> removeMessage = new RemoveMessage<Competence>(competence);
	...
	public void doSave(IProgressMonitor monitor) {
		...
		if (isNew) {
			AddMessage<Competence> addMessage = new AddMessage<Competence>(competence);
			addMessage.asnchronRequest(NetWrapper.getSession());
		}
		else {
			UpdateMessage<Competence> updateMessage = new UpdateMessage<Competence>(competence);
	...
	/**
	 * Helper method to add or update a competence
	 */
	private void addOrUpdate(Competence newCompetence) {
		// is this competence is the current one -> update it
		if (!competence.equals(newCompetence) | !competence.getCompetenceName().equals(newCompetence.getCompetenceName())) {
			return;
		}
		// save the updated competence
		setInput(new CompetenceEditorInput(newCompetence, false));
		setPartName(newCompetence.getCompetenceName());
	...
	/**
	 * Helper method to remove a competence
	 */
	private void remove(Competence removedCompetence) {
		if (!competence.equals(removedCompetence)) {
			return;
		}
		MessageDialog.openInformation(getSite().getShell(), "Kompetenz wurde gelöscht", "Die Kompetenz, welche Sie gerade editieren, wurde gelöscht");
