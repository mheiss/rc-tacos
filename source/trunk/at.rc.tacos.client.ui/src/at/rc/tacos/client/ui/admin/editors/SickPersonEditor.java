...
public class SickPersonEditor extends EditorPart implements DataChangeListener<SickPerson> {
	...
	private void createManageSection(Composite parent) {
		...
		removeHyperlink.setImage(UiWrapper.getDefault().getImageRegistry().get("admin.patientDelete"));
		removeHyperlink.addHyperlinkListener(new HyperlinkAdapter() {

			@Override
			public void linkActivated(HyperlinkEvent e) {
				boolean result = MessageDialog.openConfirm(getSite().getShell(), "Löschen des Patienten bestätigen", "Möchten sie den Patienten "
						+ person.getLastName() + " wirklich löschen?");
				if (!result)
					return;
				...
				RemoveMessage<SickPerson> removeMessage = new RemoveMessage<SickPerson>(person);
	...
	private void createDetailSection(Composite parent) {
		...
		kindOfTransportComboViewer.setInput(new String[] { IKindOfTransport.TRANSPORT_KIND_GEHEND, IKindOfTransport.TRANSPORT_KIND_TRAGSESSEL,
	...
	public void doSave(IProgressMonitor monitor) {
		...
		if (isNew) {
			...
			UpdateMessage<SickPerson> updateMessage = new UpdateMessage<SickPerson>(person);
	...
	/**
	 * Helper method to add or update a sick person
	 */
	private void addOrUpdate(SickPerson updatedPerson) {
		if (!person.equals(updatedPerson) | !updatedPerson.getLastName().equalsIgnoreCase(person.getLastName())) {
			return;
		}
		// save the updated sick person
		setInput(new SickPersonEditorInput(updatedPerson, false));
	...
	/**
	 * Helper method to remove a sick person
	 */
	private void remove(SickPerson removedPerson) {
		if (!person.equals(removedPerson)) {
			return;
		}
		MessageDialog.openInformation(getSite().getShell(), "Patient wurde gelöscht", "Der Patient, welchen sie gerade bearbeiten, wurde gelöscht");
