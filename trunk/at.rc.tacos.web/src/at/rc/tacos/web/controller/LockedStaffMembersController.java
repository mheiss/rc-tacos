...
/**
 * Locked Staff Members Controller
 * @author Birgit
 * @version 1.0
 */
public class LockedStaffMembersController extends Controller {
	...
	public Map<String, Object> handleRequest(HttpServletRequest request,
		...
		if (location != null) {
			final QueryFilter staffMemberLockedLocationFilter = new QueryFilter();
			staffMemberLockedLocationFilter.add(IFilterTypes.STAFF_MEMBER_LOCKED_LOCATION_FILTER, Integer.toString(location.getId()));
			staffMemberList = connection.sendListingRequest(StaffMember.ID, staffMemberLockedLocationFilter);
		} else {
			final QueryFilter staffMemberLockedFilter = new QueryFilter();
			String dummy = "dummy";
			staffMemberLockedFilter.add(IFilterTypes.STAFF_MEMBER_LOCKED_FILTER, dummy);
			staffMemberList = connection.sendListingRequest(StaffMember.ID, staffMemberLockedFilter);
		}
