	                    ...
	                    if(format.format(entry.getPlannedStartOfWork()).equalsIgnoreCase(format.format(today)))
	                    {
	                    		...
	                    		userSession.getStaffMember().getFirstName().toString().equals(entry.getStaffMember().getFirstName()))
	                    	{
			                    ...
			                    if(entry.getRealStartOfWork() == 0) { %>
				                       <table width="100%" height="15"><tr><td width="10" ><%=i++ %></td><td width="15" ><img src="../image/b_exit13x13.png" alt="--" /></td><td>Dienst als <%=entry.getJob().getJobName().replaceAll("ä","&auml;").replaceAll("ö","&ouml;").replaceAll("ü","&uuml;").replaceAll("ß","ss") %>&nbsp;&nbsp;-&nbsp;&nbsp;von&nbsp;<%=formatHour.format(entry.getPlannedStartOfWork()) %>&nbsp;bis&nbsp;<%=formatHour.format(entry.getPlannedEndOfWork()) %></td><td  width="20%"  ><a href="<%=request.getContextPath()%>/Dispatcher/rosterEntry.do?action=doSignIn&id=<%=entry.getRosterId()%>"><b>Dienst&nbsp;anmelden</b></a></td></tr></table>
				                <% } else { %>
