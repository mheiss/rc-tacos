...
public class RosterEntryView {
	...
	public List fillDateComboBox()
	 {
			GregorianCalendar gcal = new GregorianCalendar();
			
			List<String> content = new ArrayList();
			//the previous 5 days
			gcal.set(gcal.DATE,(gcal.get(gcal.DATE)-5));
			//up from tomorrow
			for (int i=0;i<=100;i++) // the next 100 days
			{
				gcal.set(gcal.DATE,(gcal.get(gcal.DATE))+1);
				content.add(gcal.get(gcal.DATE)+ "." +(gcal.get(gcal.MONTH)+1) +"." +gcal.get(gcal.YEAR));
			}
			return content;
	...
	public String getDefaultDate()
	{
		...
		gcal.set(gcal.DATE,(gcal.get(gcal.DATE)));
		defaultDate =gcal.get(gcal.DATE)+ "." +(gcal.get(gcal.MONTH)+1) +"." +gcal.get(gcal.YEAR);
